    ...
    for i in range(100):
        ...
        for window_x in range(0, 600, 10):
            for window_y in range(0, 300, 5):
                ...
                if test_acc == 1.0:
                    # 累加滑窗位置
                    window_final_x = window_final_x + window_x
                    window_final_y = window_final_y + window_y
                    # 记录滑窗位置
        ...
        print(windows)
        # 取所有识别的滑窗均值
        window_final_x = int(window_final_x / len(windows))
        window_final_y = int(window_final_y / len(windows))

        # 绘制最后均值滑窗锁定图像
        cv2.rectangle(frame, (window_final_x, window_final_y), (window_final_x + width, window_final_y + height),
                      (255, 0, 0), 2)
